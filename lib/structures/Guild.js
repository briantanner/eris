...
class Guild extends Base {
    constructor(data, client) {
        ...
        this.members = new LRUCollection(Member);
        if (data.members) {
            const selfMember = data.members.find(m => m.id === client.user.id);

            if (selfMember) {
                this.members.add(selfMember, this, Infinity);
            }
        }
    ...
    update(data) {
        ...
        if(data.member_count || data.memberCount) {
            this.memberCount = data.memberCount || data.member_count;
        }

        if(data.joined_at || data.joinedAt) {
            this.joinedAt = Date.parse(data.joinedAt || data.joined_at);
        }
    }
