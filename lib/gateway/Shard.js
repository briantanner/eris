    ...
    */
    ...
    identify() {
        var identify = {
            ...
            properties: {
                "os": process.platform,
                "browser": "Dyno (https://dyno.gg)",
                "device": "Dyno (https://dyno.gg)"
            }
        };
    ...
    async wsEvent(packet) {
        ...
        if (packet.d.guild_id) {
            ...
            if (guild && guild.inactive) {
                guild = await this.loadGuild(packet.d.guild_id);
            }
        }
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            case "PRESENCE_UPDATE": {
                ...
                // if (!guild) guild = this.client.guilds.get(packet.d.guild_id);
                if(!guild) {
            ...
            case "MESSAGE_UPDATE": {
                ...
                var message = channel.messages && channel.messages.get(packet.d.id);
                var oldMessage = null;
                if(message) {
                    oldMessage = {
            ...
            case "MESSAGE_REACTION_ADD": {
                ...
                var message = channel.messages && channel.messages.get(packet.d.message_id);
                if(message) {
            ...
            case "GUILD_CREATE": {
                if(!packet.d.unavailable) {
                    guild = this.createGuild(packet.d);
                    if(this.ready) {
    ...
    createGuild(_guild) {
        ...
        if (this.client.options.createGuild) {
            let guild = this.client.options.createGuild(_guild);
            return guild;
        }
    ...
    loadGuild(guildID) {
        if (!this.client.guildShardMap[guildID]) {
            this.client.guildShardMap[guildID] = this.id;
        }
        ...
        const p = new Promise(async (res, rej) => {
            let _guild = this.client.guilds.get(guildID);
            ...
            try {
                ...
                if (!guild.member_count && _guild) {
                    guild.member_count = _guild.memberCount;
                }

                if (!guild.lastActive && _guild) {
                    guild.lastActive = _guild.lastActive;
                }
    ...
    requestGuildMembers(guildID, query, limit) {
        if (this.client.options.intents && guildID.length > 1) {
            guildID.forEach((gid) => {
        ...
        switch(packet.op) {
            case OPCodes.EVENT: {
                if(!this.client.options.disableEvents[packet.t]) {
                    this.wsEvent(packet).catch(err => this.emit("error", err));
                }
                break;
            }
    ...
    editStatus(status, game) {
        ...
        this.client.guilds.forEach((guild) => {
            if(guild.shard.id === this.id) {
                if (guild.members && guild.members.has(this.client.user.id)) {
