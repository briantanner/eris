    ...
    wsEvent(packet) {
        switch(packet.t) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
            ...
            case "GUILD_UPDATE": {
                ...
                if (packet.d.name !== guild.name || 
                    packet.d.region !== guild.region || 
                    packet.d.icon !== guild.icon || 
                    packet.d.ownerID !== guild.ownerID || 
                    packet.d.verificationLevel !== guild.verificationLevel) {
                    return;
                }
