...
class Shard extends EventEmitter {
    ...
    */
    ...
    identify() {
        const identify = {
            shards: [this.id, this.id],
            events: this.client.options.events,
        };

        this.sendWS(KodoOPCodes.SUBSCRIBE, identify);
    }

    wsEvent(packet) {
        switch(packet.dType()) { /* eslint-disable no-redeclare */ // (╯°□°）╯︵ ┻━┻
    ...
    initializeWS() {
        if(!this.client.kodoToken) {
            return this.disconnect(null, new Error("Token not specified"));
        }

        this.status = "connecting";
        this.ws = new WebSocket(this.client.kodoGatewayURL, this.client.options.ws);
    ...
    onWSMessage(packet) {
        ...
        switch(packet.op) {
            case OPCodes.EVENT: {
                if(!this.client.options.disableEvents[packet.dType()]) {
                    this.wsEvent(packet);
                }
                break;
            }
            case OPCodes.HELLO: {
                ...
                * @prop {Number} id The ID of the shard
                */
                this.emit("hello", this.id);
                break; /* eslint-enable no-unreachable */
            }
            case KodoOPCodes.SUBSCRIBE_ACK: {
                this.emit("subscribed", this.id);
                break;
            }
            default: {
                this.emit("unknown", packet, this.id);
